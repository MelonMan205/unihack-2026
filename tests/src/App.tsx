...
export default function App() {
  ...
  const nearbyTonightCount = useMemo(
    () =>
      filteredEvents.filter(
        (event) => distanceInKm(event.location, userLocation) <= HERO_RADIUS_KM,
      ).length,
  ...
  return (
    <main className="relative h-[100dvh] w-full overflow-hidden bg-[#eef1f5] text-zinc-900">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-[1200] flex justify-center px-4 pt-4">
        <Card className="glass-panel pointer-events-auto w-full max-w-4xl border-white/30 text-zinc-900 animate-rise">
          <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
            <div>
              <p className="text-[11px] uppercase tracking-[0.24em] text-zinc-500">Tonight nearby</p>
              <h1 className="text-lg font-semibold text-zinc-900 sm:text-2xl">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[1200] px-4 pb-4">
        <Card className="glass-panel pointer-events-auto mx-auto w-full max-w-4xl border-white/30 animate-rise-delayed">
          <CardContent className="flex flex-wrap items-center justify-between gap-3 p-3 sm:p-4">
            ...
            <div className="pointer-events-auto relative min-w-[220px] flex-1">
              ...
              <input
                ...
                className="h-10 w-full rounded-[14px] border border-zinc-300/70 bg-white/82 pl-9 pr-3 text-sm text-zinc-800 outline-none placeholder:text-zinc-500 focus:border-zinc-400"
              />
            </div>
            <Button className="h-10 rounded-[14px] bg-yellow-400 text-zinc-900 hover:bg-yellow-300">
              Go on a side quest
            </Button>
          </CardContent>
        </Card>
        ...
        >
          {selectedEvent ? (
            <>
              ...
              <Button className="mt-6 w-full bg-yellow-400 text-zinc-900 hover:bg-yellow-300">Go now</Button>
            </>
          ) : null}
        </SheetContent>
      </Sheet>
