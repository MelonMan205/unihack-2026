...
alter table public.events
  add constraint events_price_tier_check
  check (price_tier in ('free', 'budget', 'mid', 'premium', 'unknown'));

alter table public.events
  drop constraint if exists events_alcohol_policy_check;
alter table public.events
  add constraint events_alcohol_policy_check
  check (alcohol_policy in ('alcoholic', 'non_alcoholic', 'mixed', 'unknown'));

create index if not exists events_price_tier_idx on public.events (price_tier);
create index if not exists events_alcohol_policy_idx on public.events (alcohol_policy);
create index if not exists events_is_sports_idx on public.events (is_sports);
